...
async function emitirLicencia(){
  const p = {
    buyerName:$('buyerName').value,buyerEmail:$('buyerEmail').value,
    buyerDocType:$('buyerDocType').value,buyerDocId:$('buyerDocId').value,
    payMethod:$('payMethod').value,amount:$('amount').value,
    voucherUrl:$('operationNumber').value,notes:$('notes').value
  };
  return apiIssue(p);
}
...
async function calcularDepto(){
  try {
    ...
    const d=parseInt($('depto-dormitorios').value)||0;
...
async function calcularCasa(){
  try {
    ...
    const d=parseInt($('casa-dormitorios').value)||0;
...
document.addEventListener('DOMContentLoaded', async ()=>{
  // Cargar tarifas
  try { window.TARIFAS = await apiTariffs(); console.log('Tarifas:',window.TARIFAS); }
  ...
  $('license-form')?.addEventListener('submit',async ev=>{
    ...
    } catch(e){ console.error(e); setStatus('license-status',`Error: ${e.message}`); }
  });
  ...
  $('emitir-btn')?.addEventListener('click',async ()=>{
    ...
    } catch(e){ console.error(e); setStatus('purchase-status',`Error: ${e.message}`); }
  });

  // Asignar cálculos a botones
